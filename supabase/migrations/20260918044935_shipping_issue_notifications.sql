alter table public.marsh_notification_preferences
  add column if not exists shipping_issues boolean not null default true;

update public.marsh_notification_preferences set shipping_issues = true;

insert into public.marsh_notification_preferences (user_id, channel, email, chat, billing, supplies, shipping_issues)
select id, 'none', email, true, false, false, true
from public.marsh_portal_users
on conflict (user_id) do update set shipping_issues = true;

insert into public.marsh_notification_config(key,value)
select 'email_hook_shipping_issue', value
from public.marsh_notification_config
where key='email_hook_group_message'
