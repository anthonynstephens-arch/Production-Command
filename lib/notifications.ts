 ...
 try{
  ...
  const outcomes=await Promise.all((jobs||[]).map(async(job:{id:string;recipient_id:string;channel:string;device_id:string;event_type:string;event_id:string;deduplication_key:string;attempts:number;payload:Record<string,any>;created_at:string})=>{
   try{
    ...
    let eligible=Date.now()-new Date(job.created_at).getTime()<86400000&&user?.active&&pref&&pref[job.payload.category]&&(pref.channel===job.channel||pref.channel==='both');
    if(job.event_type==='admin_broadcast')eligible=Boolean(Date.now()-new Date(job.created_at).getTime()<86400000&&user?.active&&pref&&(job.channel==='email'?pref.email:(pref.channel==='push'||pref.channel==='both')));
