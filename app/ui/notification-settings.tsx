...
type Preferences={channel:'none'|'push'|'email'|'both';email:string;chat:boolean;billing:boolean;supplies:boolean};
const defaults:Preferences={channel:'none',email:'',chat:true,billing:false,supplies:false};
 ...
 return <>
  ...
  <dialog ref={dialog} className="notification-dialog" onCancel={()=>setOpen(false)}>
   ...
   {!ready?<p>Loading settings…</p>:<>
    ...
    {preferences.channel!=='none'&&<fieldset><legend>Send me alerts about</legend>{(['chat','billing','supplies'] as const).map(key=><label key={key}><input type="checkbox" checked={preferences[key]} onChange={e=>setPreferences({...preferences,[key]:e.target.checked})}/>{({chat:'Group chat and my direct messages',billing:'Ledger charges and daily balance reminders',supplies:'Low supplies'})[key]}</label>)}</fieldset>}
    {['email','both'].includes(preferences.channel)&&<p className="notification-note">Group-message email is connected. Direct-message, billing, and low-supply emails are awaiting their email workflows. Push supports all categories.</p>}
