  ...
  return (
    <main className="dashboard">
      <header className="topbar">
        ...
        <div className="header-actions"><div className={`connection ${connected ? "live" : "demo"}`}><i />{connected ? "ShipStation connected" : "ShipStation setup needed"}</div>{session.role === "admin" && <button className="view-toggle" onClick={() => setView(view === "admin" ? "marsh" : "admin")}><ShieldCheck size={16}/>{view === "admin" ? "Admin view" : "Preview Marsh view"}</button>}{session.role === "admin" && <a className="view-toggle" href="#portal-users" onClick={()=>setView("admin")}>Manage users</a>}<button className="view-toggle" onClick={logout}>{session.name} · Sign out</button></div>
        {mobileMenuOpen && <div className="mobile-menu">
          ...
          <nav aria-label="Mobile dashboard sections">
            {session.role === "admin" && <a href="#portal-users" onClick={()=>{setView("admin");setMobileMenuOpen(false)}}>Manage users</a>}<a href="#inventory" onClick={() => setMobileMenuOpen(false)}>Inventory</a>
      ...
      <div className="page-shell"><div className="notification-toolbar"><NotificationSettings isAdmin={session.role === "admin"}/></div>
        ...
        <section className="metrics-grid">
          ...
          <article className="metric issues-metric"><div className="metric-heading"><span className="metric-icon red"><Flag size={23}/></span><p>Orders with issues</p></div><div className="metric-value"><strong>{orderIssues.length}</strong><small>Resolve issues to ship</small></div></article>
        </section>
